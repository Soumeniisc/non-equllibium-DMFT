...
module PARM_MOD
  ...
  type, public :: parm
     ...
     double precision   :: delta_i            ! # initial ionic potetial 
     double precision   :: delta_f            ! # final ionic potetial 
...
contains
  ...
  subroutine set_parm(parm_)
    ...
    !parm_%init = 1
    call getarg(14,arg)
    read(arg,*) parm_%delta_i
    call getarg(15,arg)
    read(arg,*) parm_%delta_f
  ...
  double precision function delta_(parm_,t)
    type(parm), intent(in)       :: parm_
    double precision, intent(in) :: t
    
    delta_=parm_%delta_f
