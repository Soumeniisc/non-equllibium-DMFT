...
contains
  ...
  subroutine start_noneq_dmft(parm_,dmft_)
    ...
    open (unit=1112, file='interaction', action='write')
    write (1112,*) dble(1-1)*parm_%dt, U(parm_,dble(1-1)*parm_%dt)
    open (unit=1113, file='delta', action='write')
    write (1113,*) dble(1-1)*parm_%dt, parm_%delta_i
    ...
    write (1112,*) dble(n-1)*parm_%dt, U(parm_,dble(n-1)*parm_%dt)
    write (1113,*) dble(1-1)*parm_%dt, delta_(parm_,dble(n-1)*parm_%dt)
    end do
    close(1112)
    close(1113)
