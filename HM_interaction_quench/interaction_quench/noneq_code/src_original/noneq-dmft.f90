...
module NONEQ_DMFT_MOD
  ...
  use IPT_MOD
  use INTEGRAL_MOD
  use FFT_MOD  ! will be used local_green_e intialisation
  implicit none
...
contains
  ...
  subroutine start_noneq_dmft(parm_,dmft_)
    ...
    call initialize_noneq_green_e_positive(parm_,dmft_)
    !call initialize_noneq_green_e_negative(parm_,dmft_)
    do n=2, parm_%N_t+1 ! maximum time is t=t_{n}=(n-1)*dt
       ...
       !call measure_dn2(parm_,dmft_)
       !call measure_dn(parm_,dmft_)
       call initialize_noneq_green_e_positive(parm_,dmft_)
    end do
...
!/////////////////////////
  ...
  subroutine measure_dn(parm_,dmft_) ! this program able to capture correct dn(t) when there is no initialize_noneq_green_e_negative
    type(parm), intent(in)          :: parm_
    type(dmft), intent(inout)       :: dmft_
    integer                         :: n,l,i
    ...
    if (n==2) then
       ...
       dmft_%local_green_der_e%retarded(1)=0.0

       open (unit=7, file='dn', status='old', action='write', position='append')
       write (7,'(f15.10,f15.10,f15.10,f15.10,f15.10)') 0.0, Real(dmft_%local_green_e%retarded(1,1)),imag(dmft_%local_green_e%retarded(1,1)), real(dmft_%local_green_der_e%retarded(1)),imag(dmft_%local_green_der_e%retarded(1))
       close (7)
    end if
    ...
    !                    -i\int_{t'}^{t} ds K^{R}(t,s)*G^{R}(s,t')
       dmft_%local_green_e%retarded(n,1) = dmft_%local_green_e%retarded(n-1,1) + 0.5d0*parm_%dt*dmft_%local_green_der_e%retarded(1)
       do l=1, n-1
        KxG(l)=dmft_%self_energy%retarded(n,l)*dmft_%local_green_e%retarded(l,1)
       end do
       dmft_%local_green_der_e_new%retarded(1) = -xj*parm_%dt*trapezoid_half_edge(KxG,1,n-1)
       print*, "t, trapizoidal",t, dmft_%local_green_der_e_new%retarded(1)
       dmft_%local_green_e%retarded(n,1) = dmft_%local_green_e%retarded(n,1) + 0.5d0*parm_%dt*dmft_%local_green_der_e_new%retarded(1)
       ...
       dmft_%local_green_der_e_new%retarded(1) = dmft_%local_green_der_e_new%retarded(1) -xj*h(n)*dmft_%local_green_e%retarded(n,1) -0.5d0*xj*parm_%dt*dmft_%local_green_e%retarded(n,1)*dmft_%self_energy%retarded(n,n)   
       dmft_%local_green_der_e%retarded(1)=dmft_%local_green_der_e_new%retarded(1)
       deallocate(KxG)

    open (unit=7, file='dn', status='old', action='write', position='append')
    write (7,'(f15.10,f15.10,f15.10,f15.10,f15.10)') t, REAL(dmft_%local_green_e%retarded(n,1)), IMAG(dmft_%local_green_e%retarded(n,1)),real(dmft_%local_green_der_e_new%retarded(1)),imag(dmft_%local_green_der_e_new%retarded(1))
    close (7)

    !call deallocate_KB_derivative(dmft_%local_green_der_e)
    !call allocate_KB_derivative(parm_,dmft_%local_green_der_e,n)
    do l=1, n
    ...
    !call allocate_KB_derivative(parm_,dmft_%local_green_der_e_new,n+1)
    
  end subroutine measure_dn

!/////////////////////////////////
 subroutine measure_dn2(parm_,dmft_) ! this program able to capture correct dn(t) when there is no initialize_noneq_green_e_negative
    type(parm), intent(in)          :: parm_
    type(dmft), intent(inout)       :: dmft_
    integer                         :: n,l,i
    double precision                :: t,dn,dnd ! dn = G^R(t,0) and dnd = d/dt G^R(t,0)
    ...
    if (n==2) then
       ...
       open (unit=7, file='dn_vol', status='old', action='write', position='append')
       write (7,'(f15.10,f15.10,f15.10,f15.10,f15.10)') 0.0, Real(dmft_%local_green_e%retarded(1,1)),imag(dmft_%local_green_e%retarded(1,1)), real(dmft_%local_green_der_e%retarded(1)),imag(dmft_%local_green_der_e%retarded(1))
       close (7)
  ...
  end subroutine measure_dn2

!/////////////////////////////////////////////////////////////////////
 subroutine initialize_noneq_green_e_negative(parm_,dmft_)
    type(parm), intent(in)          :: parm_
    type(dmft), intent(inout)       :: dmft_
    integer                         :: i, j, k, n
    real                            ::  epsilon_,w
    ...
    !intialising matubara grrwn function at epsilon
    epsilon_ = 0.0001
    do j=1, parm_%N_tau
       w=dble(2*j-parm_%N_tau-1)*pi/parm_%beta
       dmft_%local_green_e%matsubara_w(j)=1.d0 / (xj*w - epsilon_ - dmft_%self_energy%matsubara_w(j))
    ...
    open (unit=7, file='n_lesser_negetive', status='old', action='write', position='append')
    write (7,'(f15.10,f15.10,f15.10)') 0.0, REAL(dmft_%local_green_e%lesser(1,1)), IMAG(dmft_%local_green_e%lesser(1,1))
    close (7) 
 end subroutine initialize_noneq_green_e_negative
