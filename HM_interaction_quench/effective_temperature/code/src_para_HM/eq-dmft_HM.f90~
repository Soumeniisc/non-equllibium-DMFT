...
contains
  ...
  subroutine start_eq_dmft(parm_,dmft_)
    ...
    ! call initialize_self_energy(parm_,dmft_) !it initialise contour self energy and other self energy like retaded, lesser, left.
  ...
  subroutine measure_total_energy(parm_,dmft_)
   ...
   subroutine print_green_function(parm_,dmft_)
    ...
    open (unit=13, file='Gf', status='old', action='write')
    do j=1, parm_%N_tau+1
        tau=dble(j-1)*parm_%dtau
        write (12,*) tau, dmft_%local_green%matsubara_t(j)
    end do    
    close (12)
